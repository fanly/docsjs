...
function parseNumberingMap(numberingXml: Document | null): Map<string, NumberingLevelSpec> {
  ...
  for (const abs of abstractNums) {
    ...
    for (const lvl of lvlNodes) {
      ...
      lvlMap.set(ilvl, {
        ...
        startAt:
          toInt(
            getAttr(queryAllByLocalName(lvl, "start")[0] ?? null, "w:val") ??
              getAttr(queryAllByLocalName(lvl, "start")[0] ?? null, "val"),
          ) ?? 1,
      });
    }
  ...
  for (const num of nums) {
    ...
    for (const override of lvlOverrides) {
      ...
      const overrideNumFmtNode = overrideLvl
        ? (queryAllByLocalName(overrideLvl, "numFmt")[0] ?? null)
        : null;
      const overrideLvlTextNode = overrideLvl
        ? (queryAllByLocalName(overrideLvl, "lvlText")[0] ?? null)
        : null;
      ...
      levelMap.set(key, {
        numFmt:
          getAttr(overrideNumFmtNode, "w:val") ?? getAttr(overrideNumFmtNode, "val") ?? base.numFmt,
        lvlText:
          getAttr(overrideLvlTextNode, "w:val") ??
          getAttr(overrideLvlTextNode, "val") ??
          base.lvlText,
...
function parseParagraphProfiles(
  documentXml: Document,
  numberingMap: Map<string, NumberingLevelSpec>,
  ...
  return paragraphs.map((paragraph, index) => {
    ...
    const listSpec =
      listNumId !== null && listLevel !== null
        ? numberingMap.get(`${listNumId}:${listLevel}`)
        : undefined;
    ...
    const pageBreakBeforeNode = pPr
      ? (queryAllByLocalName(pPr, "pageBreakBefore")[0] ?? null)
      : null;
    const renderedPageBreakNode =
      queryAllByLocalName(paragraph, "lastRenderedPageBreak")[0] ?? null;
    ...
    const rawLineRule = (
      getAttr(spacing, "w:lineRule") ??
      getAttr(spacing, "lineRule") ??
      "auto"
    ).toLowerCase();
    ...
    return {
      ...
      keepNext:
        keepNextNode !== null &&
        (getAttr(keepNextNode, "w:val") ?? getAttr(keepNextNode, "val") ?? "1") !== "0",
      keepLines:
        keepLinesNode !== null &&
        (getAttr(keepLinesNode, "w:val") ?? getAttr(keepLinesNode, "val") ?? "1") !== "0",
      pageBreakBefore:
        renderedPageBreakNode !== null ||
        (pageBreakBeforeNode !== null &&
          (getAttr(pageBreakBeforeNode, "w:val") ?? getAttr(pageBreakBeforeNode, "val") ?? "1") !==
            "0"),
...
function parseTableDefaults(stylesXml: Document): {
  ...
  const targetStyle =
    tableStyles.find((style) => {
      const styleId = (
        getAttr(style, "w:styleId") ??
        getAttr(style, "styleId") ??
        ""
      ).toLowerCase();
      return styleId === "a1";
    }) ??
...
function parseRunProfiles(paragraph: Element): RunStyleProfile[] {
  ...
  return runNodes
    .map((run) => {
      ...
      const highlightRaw = (
        getAttr(highlightNode, "w:val") ??
        getAttr(highlightNode, "val") ??
        ""
      ).toLowerCase();
      ...
      const highlightColor =
        highlightRaw && highlightRaw !== "none" ? (highlightMap[highlightRaw] ?? null) : null;
      ...
      const spacingVal =
        getTwipAttr(spacingNode, "w:val") ?? getTwipAttr(spacingNode, "val") ?? null;
      ...
      const bold =
        bNode !== null && (getAttr(bNode, "w:val") ?? getAttr(bNode, "val") ?? "1") !== "0";
      const italic =
        iNode !== null && (getAttr(iNode, "w:val") ?? getAttr(iNode, "val") ?? "1") !== "0";
      const underlineVal = (getAttr(uNode, "w:val") ?? getAttr(uNode, "val") ?? "").toLowerCase();
      const underline = uNode !== null && underlineVal !== "none";
      const strike =
        strikeNode !== null &&
        (getAttr(strikeNode, "w:val") ?? getAttr(strikeNode, "val") ?? "1") !== "0";
      const shadow =
        shadowNode !== null &&
        (getAttr(shadowNode, "w:val") ?? getAttr(shadowNode, "val") ?? "1") !== "0";
      const vertAlign = (
        getAttr(vertAlignNode, "w:val") ??
        getAttr(vertAlignNode, "val") ??
        ""
      ).toLowerCase();
...
function parseDefaults(stylesXml: Document): {
  ...
  if (!docDefaults) {
    return {
      bodyFontPx: null,
      bodyLineHeightRatio: null,
      bodyLineHeightPx: null,
      bodyLineHeightRule: "auto",
      paragraphAfterPx: null,
    };
  }
