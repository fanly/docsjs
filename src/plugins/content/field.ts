...
export function createFieldPlugin(): ParagraphPlugin {
  return {
    ...
    parseParagraph(element: Element, _context: PluginContext): ParagraphParseResult {
      const fldCharBegins = Array.from(
        element.querySelectorAll('w\\:fldChar[fldCharType="begin"], fldChar[fldCharType="begin"]'),
      );
      ...
      for (const begin of fldCharBegins) {
        ...
        if (instrText) {
          ...
          if (instr.startsWith("PAGE ")) {
            ...
            type = "totalPages";
          } else if (instr.startsWith("DATE ")) {
            type = "date";
          } else if (instr.startsWith("TIME ")) {
            type = "time";
          } else if (instr.startsWith("TOC")) {
            type = "toc";
          } else if (instr.startsWith("AUTHOR")) {
            type = "author";
          } else if (instr.startsWith("TITLE")) {
            type = "title";
          } else if (instr.startsWith("FILENAME")) {
            type = "fileName";
          }
      ...
      const fieldHtml = fields
        .map((f) => `<span data-word-field="${f.type}" data-word-field-instr="${f.instr}"></span>`)
        .join("");
