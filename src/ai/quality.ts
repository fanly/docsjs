...
export class QualityPredictionModel {
  ...
  private calculateStructureScore(chars: DocumentCharacteristics): number {
    ...
    // Headings
    if (chars.headingCount > 0) {
      score += 15;
    }
    if (chars.headingCount > 3) {
      score += 10;
    }

    // Paragraphs
    if (chars.paragraphCount > 5) {
      score += 10;
    }
    if (chars.avgParagraphLength > 30 && chars.avgParagraphLength < 150) {
      score += 10;
    }
  ...
  private calculateFormattingScore(chars: DocumentCharacteristics): number {
    ...
    // Images
    if (chars.imageCount > 0) {
      score += 10;
    }
    if (chars.imageCount > 3) {
      score += 10;
    }

    // Tables
    if (chars.tableCount > 0) {
      score += 10;
    }

    // Lists
    if (chars.listCount > 0) {
      score += 10;
    }
  ...
  private calculateContentScore(chars: DocumentCharacteristics): number {
    ...
    if (chars.wordCount > 1000) {
      score += 10;
    }

    // Links
    if (chars.linkCount > 0) {
      score += 10;
    }

    // Metadata
    if (chars.hasMetadata) {
      score += 10;
    }
  ...
  private calculateConsistencyScore(chars: DocumentCharacteristics): number {
    ...
    if (chars.paragraphCount > 10) {
      score += 10;
    }
    if (chars.avgParagraphLength > 20 && chars.avgParagraphLength < 200) {
      score += 10;
    }
