...
export class SSOService {
  ...
  async getOAuthAuthorizationUrl(provider: SSOProvider, state: string): Promise<string> {
    ...
    const authUrl = config.authorizationEndpoint || `${config.issuer}/authorize`;
...
<samlp:AuthnRequest 
   ...
   */
  async processSAMLResponse(provider: SSOProvider, samlResponse: string): Promise<SSOUserProfile> {
    const config = this.samlConfigs.get(provider);
    if (!config) {
      throw new Error(`SAML not configured for provider: ${provider}`);
    }
