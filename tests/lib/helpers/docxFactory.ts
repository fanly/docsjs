...
export async function makeDocxFile(input: DocxFactoryInput): Promise<File> {
  ...
  if (input.footnotesXml) {
    zip.file("word/footnotes.xml", input.footnotesXml);
  }
  if (input.endnotesXml) {
    zip.file("word/endnotes.xml", input.endnotesXml);
  }
  if (input.commentsXml) {
    zip.file("word/comments.xml", input.commentsXml);
  }
