...
const demoHooksPlugin: DocxPlugin = {
  ...
  afterParse: (ctx) => {
    if (ctx.pipeline.state.ast) {
      ...
      ctx.pipeline.state.metadata.detectedElements = Object.keys(
        ctx.pipeline.state.ast.metadata.elements || {},
      );
    }
    return ctx;
  },
...
export async function runCompleteVerification(): Promise<boolean> {
  ...
  try {
    ...
    if (secPerms.network !== false) {
      throw new Error("Network access not disabled");
    }
    if (secPerms.compute.maxMemoryMB !== 5) {
      throw new Error("Memory limit not enforced");
    }
    if (secPerms.ast.canExportRawAst !== false) {
      throw new Error("AST export not blocked");
    }
...
// For exports
export { runCompleteVerification, runPlatformSimulation, demoHooksPlugin, demoSecurityPlugin };
