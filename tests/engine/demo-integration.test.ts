...
import { describe, it, expect, beforeEach, vi, afterEach, beforeAll } from "vite-plus/test";
...
describe("Demo Framework Integration", () => {
  ...
  it("should be compatible with plugin patterns used in existing integrations", () => {
    ...
    const webIntegrationPlugin = {
      ...
      availableHooks: ["beforeRender", "afterExport"] as const,
  ...
  it("should validate plugin permissions against engine security policy", () => {
    ...
    expect(() => strictEngine.registerPlugin(pluginWithNetAccess)).toThrow(
      "Plugin requires network access but engine network is disabled",
    );
